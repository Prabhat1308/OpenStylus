...
impl<T: ERC721Params> ERC721<T> {
    ...
    fn require_authorized_to_spend(&self, from: Address, token_id: U256) -> Result<()> {
        ...
        Err(
            ERC721Error::NotApproved(NotApproved {
                owner,
                spender: msg::sender(),
                token_id,
            })
    ...
    fn call_receiver<S: TopLevelStorage>(
            ...
            hash == b256!("c5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470")
        {
            ...
            let received = receiver.on_erc_721_received(
                storage,
                msg::sender(),
                from,
                token_id,
    ...
    pub fn batch_safe_mint(&mut self,storage: &mut S, to: Vec<Address>, token_ids: Vec<U256>, data: Vec<u8>)->Result<()>{
        for i in token_ids
        storage.borrow_mut().mint(to[i], token_id[i])?;
        Self::call_receiver(storage, token_id, Address::default(), to[i], data[i])?;
        Ok(())
    }
    ...
    pub fn batch_burn(&mut self, token_ids: Vec<U256>)->Result<()>{
        for i in token_ids
        self.burn(token_id[i])?;
        Ok(())
    }
...
impl<T: ERC721Params> ERC721<T> {
    ...
    pub fn uri(&self, token_id: U256) -> Result<String> {
    ...
    pub fn batch_safe_transfer_from<S: TopLevelStorage + BorrowMut<Self>>(
        storage: &mut S,
        from: Address,
        to: Address,
        token_ids: Vec<U256>
        ...
        for i in token_ids {
        Self::safe_transfer_from_with_data(storage, from, to, Bytes(token_ids[i]), Bytes(vec![]))
        }
    ...
    pub fn balance_of_batch(&self, owners: Vec<Address>)-> Result<Vec<U256>>{
        response = Vec::new()
        for i in owners
        response.push(U256::from(self.balance.get(owner[i])))
    ...
    pub fn set_approval_for_all(&mut self, operator: Address, approved: bool) -> Result<()> {
        ...
        self.approved_for_all.setter(owner).insert(operator, approved);
